...
function texMath(state: any, silent: boolean) {
  let startMathPos = state.pos
  if (state.src.charCodeAt(startMathPos) !== 0x25 /* % */) {
    return false
  }
  ...
  if (!silent) {
    const token = state.push(
      endMarker.length === 1 ? 'inline_am' : 'display_am',
      '',
      0,
    )
...
export default (md: MarkdownIt) => {
  md.inline.ruler.push('amMath', texMath)
  md.renderer.rules.inline_am = (tokens: any[], idx: number) =>
    latex.inlineAm(tokens[idx].content)
  md.renderer.rules.display_am = (tokens: any[], idx: number) =>
    latex.displayAm(tokens[idx].content)
}

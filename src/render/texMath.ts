...
function texMath(state: any, silent: boolean) {
  ...
  if (!silent) {
    const token = state.push(
      endMarker.length === 1 ? 'inline_latex' : 'display_latex',
      '',
      0,
    )
...
export default (md: MarkdownIt) => {
  ...
  md.renderer.rules.inline_latex = (tokens: any[], idx: number) =>
    latex.inline(tokens[idx].content)
  md.renderer.rules.display_latex = (tokens: any[], idx: number) =>
    latex.display(tokens[idx].content)
}
